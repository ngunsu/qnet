...
require 'nn'			
local DistanceRatioCriterion2, parent = torch.class('nn.DistanceRatioCriterion2', 'nn.Criterion')

function DistanceRatioCriterion2:__init()
    parent.__init(self)
...
function DistanceRatioCriterion2:createTarget(input, target)
...
function DistanceRatioCriterion2:updateOutput(input, target)
...
function DistanceRatioCriterion2:updateGradInput(input, target)
    if not self.Target:isSameSizeAs(input) then
        self:createTarget(input, target)
    end
...
function DistanceRatioCriterion2:type(t)
    parent.type(self, t)
    self.SoftMax:type(t)
    self.MSE:type(t)
